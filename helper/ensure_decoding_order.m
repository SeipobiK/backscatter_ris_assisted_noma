    ...
    for c = 1:K
        ...
        for j = 1:K
            if j ~= c
                ...
                inter_cluster_near_b = inter_cluster_near_b + ...
                    abs(g_local(:, c, 2)' * J_r * Theta * J_t * G_all_matrix * f_local(c, 1) * w_k(:, j))^2*eta_k;
            end
        end
        
        % Equivalent combined channel gains (following paper's Eq. 11)
        gamma_n = abs(H_n_combined)^2/(abs(H_n_combined_c)^2*eta_k + inter_cluster_near_b + inter_cluster_near  + noise);
