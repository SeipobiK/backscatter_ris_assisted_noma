function [V_opt,A_n_opt, B_n_opt, A_f_opt, B_f_opt, A_c_n_opt, B_c_n_opt,obj_history,obj_history_mc,converged] =passive_BF_opt(para,w_k,G_all, g_1_all,...
    ...
    [V_opt_init,A_n_opt, B_n_opt, A_f_opt, B_f_opt, A_c_n_opt, B_c_n_opt,obj_curr,status] = relaxed_passive_2(para,w_k,G_all, g_1_all,...
    ...
    step_size(1) = (1-initial_ratio)*0.8;
    ...
    for m = 2:max_iter
        ...
        if ~strcmp(status, 'Solved')
         ...
         [V_opt,A_n_opt, B_n_opt, A_f_opt, B_f_opt, A_c_n_opt, B_c_n_opt,obj_prev,cvx_status] = Passive_BF_2(para,w_k,G_all, g_1_all,...
            g_2_all,g_b_all,f1_all,f2_all,A_n_prev, B_n_prev, A_f_prev, B_f_prev,  A_c_prev_n, B_c_prev_n,max_eigVector_V_opt(:,m-1),relax_parameter(m-1),alpha_f,alpha_n);
        ...
        current_ratio = max_eigenvalue_V_opt(m)/trace(U_opt(:,:,m));
            ...
            if  obj_history(m) - obj_history(m-1)<0
                  ...
                  relax_parameter(m) = min(1, current_ratio + 0.76*(1-current_ratio)); % 10% step
            end

        % Check convergence
        if m > 1 && abs(obj_history(m) - obj_history(m-1)) < 1e-3 && strcmp(cvx_status, 'Solved') && abs(1-relax_parameter(m)) <= 1e-4
             converged = true;
             obj_history = obj_history(1:m); 
            break;
        end
