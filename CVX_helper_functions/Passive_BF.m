function [V_opt,A_n_opt, B_n_opt, A_f_opt, B_f_opt, A_c_n_opt, B_c_n_opt,obj_prev,status] = Passive_BF(para,w_k,G_all, g_1_all,...
    g_2_all,g_b_all,f1_all,f2_all,A_n_prev, B_n_prev, A_f_prev, B_f_prev,  A_c_prev_n, B_c_prev_n,V_max,epsln_1,alpha_f,alpha_n)
   ...
   for c=1:numClusters
         ...
         H_n_c{c}  = diag(g_b_all{c}')*f1_all{c}*G_all*w_k(:, c);
         H_f_c{c}  = diag(g_b_all{c}')*f2_all{c}*G_all*w_k(:, c);
   end
   ...
   cvx_begin quiet sdp
    ...
    %    cvx_precision high
       ...
       maximize( para.weights_n * sum(R_n + R_f) )
