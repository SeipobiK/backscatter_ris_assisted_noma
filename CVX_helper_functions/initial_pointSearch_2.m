function [V_opt,A_n_opt, B_n_opt, A_f_opt, B_f_opt, A_c_n_opt, B_c_n_opt,obj_prev,status] = initial_pointSearch_2(para,w_k,G_all, g_1_all,...
               ...
               H_f_c{c}=0;
                ...
                %                                     (log2(exp(1)) / (B_c_prev_n(c) * (1 + A_c_prev_n(c) * B_c_prev_n(c)))) * (B_c_n(c) - B_c_prev_n(c));
                   ...
                   for j = 1:numClusters
                       ...
                       inv_pos(A_n(c))- real(trace(V  * H_n{c} * H_n{c}')) * alpha_n(c)<=delta_p; % Near user
